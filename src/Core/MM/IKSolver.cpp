...
namespace mm
{
	...
	{
		m_nodes[pivot].local.rot = m_nodes[pivot].local.rot * qL;

		for (int32_t i = pivot; i >= 0; --i) {
			glm::vec3 offset = m_nodes[i].world.trans - m_nodes[pivot].world.trans;
			m_nodes[i].world.trans = m_nodes[pivot].world.trans + glm::rotate(qW, offset);
			m_nodes[i].world.rot = qW * m_nodes[i].world.rot;
		}
	}
	...
	void IKSolver::Solve()
	{
		for (uint32_t iteration = 0; iteration < m_ik.iteration; ++iteration) {
			for (uint32_t i = 1; i < m_nodes.size(); ++i) {
				glm::vec3 o = m_nodes[i].world.trans;
				glm::vec3 E = m_nodes.front().world.trans;
				glm::vec3 T = m_armature.m_bones[m_target].animWorld.trans;
				glm::vec3 t = glm::normalize(T - o);
				glm::vec3 e = glm::normalize(E - o);
				glm::quat qW = glm::rotation(e, t);
				glm::quat qLtoW = m_nodes[i].world.rot;
