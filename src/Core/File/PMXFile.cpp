...
PMXFile::PMXFile(const Path& path) :
	File{ path }
{
	InFileStream stream{};
...
String PMXFile::ParseText(InFileStream& stream)
{
	u32 length{};
	Read(stream, length);
	...
	switch (static_cast<PMXFile::TextEncoding>(m_header.textEncoding)) { 
...
void PMXFile::ParseHeader(InFileStream& stream)
{
	Read(stream, m_header.magic);
	if (memcmp(m_header.magic, "PMX ", 4)) {
		throw FileLoadError("Invalid PMX file");
	}
...
void PMXFile::ParseInfo(InFileStream& stream)
{
	m_info.nameJP = ParseText( stream);
	m_info.nameEN = ParseText( stream);
	m_info.commentJP = ParseText( stream);
	m_info.commentEN = ParseText( stream);
}

void PMXFile::ParseVertex(InFileStream& stream)
{
	i32 vertexCount{};
	Read(stream, vertexCount);
	m_vertices.resize(vertexCount);
	for (i32 i = 0; i < vertexCount; ++i) {
		PMXFile::Vertex& vertex = m_vertices[i];
		...
		break;
		default:
			throw FileLoadError("Unknown blending type");
		}
...
void PMXFile::ParseFace(InFileStream& stream)
{
	i32 elementCount{};
	...
	for (i32 i = 0; i < elementCount; ++i) {
		PMXFile::Face& face = m_faces[i];
		Read(stream, face.vertexIndices, m_header.vertexIndexSize);
	}
}

void PMXFile::ParseTexture(InFileStream& stream)
{
	i32 textureCount{};
	...
	for (i32 i = 0; i < textureCount; ++i) {
		PMXFile::Texture& texture = m_textures[i];
		texture.name = ParseText( stream);
	}
}

void PMXFile::ParseMaterial(InFileStream& stream)
{
	i32 materialCount{};
	Read(stream, materialCount);
	m_materials.resize(materialCount);
	for (i32 i = 0; i < materialCount; ++i) {
		PMXFile::Material& material = m_materials[i];
...
void PMXFile::ParseMorph(InFileStream& stream)
{
	...
	for (i32 i = 0; i < morphCount; ++i) {
		...
		for (i32 j = 0; j < offsetCount; ++j) {
			...
			break;
			default:
				throw FileLoadError("Unknown morph type");
			}
		}
	}
}

void PMXFile::ParseFrame(InFileStream& stream)
{
	i32 frameCount{};
	Read(stream, frameCount);
	m_clusters.resize(frameCount);
	for (i32 i = 0; i < frameCount; ++i) {
		PMXFile::Cluster& cluster = m_clusters[i];
		...
		for (i32 j = 0; j < elementCount; ++j) {
			...
			case PMXFile::ClusterType::MORPH:
				...
				break;
			default:
				throw FileLoadError("Unknown cluster type");
			}
		}
	}
}

void PMXFile::ParseRigidbody(InFileStream& stream)
{
	i32 rigidbodyCount{};
	...
	for (i32 i = 0; i < rigidbodyCount; ++i) {
		PMXFile::Rigidbody& rigidbody = m_rigidbodies[i];
...
void PMXFile::ParseJoint(InFileStream& stream)
{
	i32 jointCount{};
	...
	for (i32 i = 0; i < jointCount; ++i) {
		PMXFile::Joint& joint = m_joints[i];
